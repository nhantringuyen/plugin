...
class WPFTS_Result_Item
{
	...
	public function Excerpt($query = '')
	{
		...
		if (($r1['is_not_found_words']) && is_array($r1['not_found_words']) && (count($r1['not_found_words']) > 0)) {
			...
			$a['not_found_words'] = '<div class="wpfts-not-found"><span>'.__('Not found', 'wpfts_lang').': '.implode(' ', $nfs).'</span></div>';
		}
		$a['score'] = '';
		if ($r1['is_score']) {
			$a['score'] = '<span class="wpfts-score">'.__('Score', 'wpfts_lang').': '.number_format_i18n($r1['score'], 2).'</span>';
		}
		$a['link'] = '';
		if ($r1['is_attachment']) {
			...
			if ($r1['is_direct_link']) {
				$a['link'] = '<a class="wpfts-download-link'.$shift.'" href="'.esc_url($r1['link']).'"><span>'.__('Download', 'wpfts_lang').( $r1['is_filesize'] ? ' ('.size_format(floatval($r1['filesize']), 2).')' : '').'</span></a>';
			} else {
				if ($r1['is_filesize']) {
					$a['link'] = '<span class="wpfts-file-size'.$shift.'">'.__('File Size', 'wpfts_lang').': '.size_format(floatval($r1['filesize']), 2).'</span>';
				}
			}
