            ...
            <p class="form-field" <?php echo $data ?>>
                ...
                <?php switch( $field['type'] ) :
                        ...
                        case 'variation_select': ?>
                            <select id="<?php echo esc_attr( $field['name'] ) ?>" name="<?php echo esc_attr( $field['name'] ) ?>" <?php echo $attr ?>>
                                <?php
                                ...
                                foreach( $variations as $variation ) :
                                    ...
                                    ?>
                                    <option value="<?php echo $variation[ 'id' ] ?>" <?php selected( $variation[ 'id' ], $value ) ?>><?php echo $variation[ 'name' ] ?></option>
                                <?php
                                endforeach;
                                ?>
                            </select>
                        ...
                        case 'select': ?>
                            <select id="<?php echo esc_attr( $field['name'] ) ?>" name="<?php echo esc_attr( $field['name'] ) ?>" <?php echo $attr ?>>
                                <?php foreach( $field['option'] as $option_key => $option_name ) : ?>
                                    <option value="<?php echo $option_key ?>" <?php selected( $option_key, $value ); ?>><?php echo $option_name; ?></option>
                                <?php endforeach; ?>
                            </select>
                            <?php break;

                    case 'textarea' : ?>
                        <textarea id="<?php echo esc_attr( $field['name'] ) ?>" name="<?php echo esc_attr( $field['name'] ) ?>" <?php echo $attr ?>><?php echo esc_html( $value ); ?></textarea>
                        <?php break;

                        case 'checkbox': ?>
                            <input type="checkbox" id="<?php echo esc_attr( $field['name'] ) ?>" name="<?php echo esc_attr( $field['name'] ) ?>" value="yes" <?php checked( 'yes', $value ) ?> <?php echo $attr ?>/>
                        ...
                        default: ?>
                            <input type="<?php echo esc_attr( $field['type'] ) ?>" class="<?php echo $class ?>" id="<?php echo esc_attr( $field['name'] ) ?>" name="<?php echo esc_attr( $field['name'] ) ?>"
                                   value="<?php echo $value ?>" <?php echo $attr ?>/>
                            <?php if( $desc ) : ?>
                                <img class="help_tip" data-tip="<?php echo $desc; ?>" src="<?php echo WC()->plugin_url(); ?>/assets/images/help.png" height="16" width="16"/>
                            <?php endif; ?>
                            <?php break;
